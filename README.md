...
参考文献：配置了符合学校要求的 BibLatex 参考文献格式（GB/T 7714-2015，**并将参考文献部分的序号字体改为宋体（当然老师说不需要的话也提供了开关一键关闭）**）。
...
目前的模板只能使用Windows的TeX Live编译器，暂时没有经过Overleaf **（本模板在Overleaf上编译会超出免费编译时长，为各位省钱就不做Overleaf版本适配了）** 和Mac系统 **（作者身边也没有用Mac的同学...）** 的测试。
